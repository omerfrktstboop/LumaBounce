...
LumaBounce is a Godot 4.7 portrait-mobile physics bounce-puzzle game (GDScript, GL Compatibility renderer). The player drags back a fixed launcher to fire a ball that bounces off panels/walls/obstacles into a target. 155 hand- and AI-authored levels (150 campaign + 5 bonus), no external art/audio assets — everything is drawn procedurally (`ShapeBuilder`, `_draw()`) and synthesized (`tools/generate_sfx.py`).
...
Source language is **Turkish**; `Locale` is the single exit point for `TranslationServer.set_locale` (same pattern and rationale as `Haptics`). Two CSV tables: `ui.csv` (interface) and `levels.csv` (level names plus tutorial strings). Launch locales are `tr, en, es, pt_BR, de, fr` — the `ui.csv` header row is the authoritative list.

**The translation key is the Turkish source string itself**, not a symbolic id. This is a deliberate maintenance trade-off: `LEVEL_042_NAME`-style keys would require touching all 155 `.tres` files and would make them unreadable, and the level generator writes `display_name` itself. With source-as-key, an untranslated string renders as *Turkish*, never blank.

Note that level names are **not** all distinct: the 155 level files use 136 distinct `display_name` values. Source-as-key handles this correctly — two levels sharing a Turkish name share one translation row, which is the desired result — but it does mean a level name is not a unique identifier. Use `level_id` for that.
...
### Home/Shop/Settings design system (FAZ 9, `scripts/ui/*`)
`UIMetrics` (`scripts/ui/ui_metrics.gd`) is a token-only sibling of `Palette`: spacing, corner radius and a font-size scale as plain `const`s (no `apply_theme()` — these never change per level band, unlike `Palette`'s colors). Font sizes are calibrated to the game's existing ~720-unit reference width, **not** to the brand guide's literal px values, which were written for a different base unit; using them verbatim would have shrunk on-screen text to roughly half its current size.

Home/Shop/Settings share a small component set instead of each screen hand-building `StyleBoxFlat`s: `LumaCard` (static style builder — consolidates three previously-diverged card patterns: `shop_screen.gd`, `pause_card.gd`, `mechanic_intro_card.gd` had different radii and fill colors; only the Faz-9 screens were migrated, `pause_card.gd`/`mechanic_intro_card.gd` are an obvious later cleanup), `IconTile`, `CoinChip`, `NavigationCard`, `ProductCard`, `SegmentedControl`, `ToggleSwitch`.

**Button subclasses with manually-added children must set `custom_minimum_size` explicitly, not rely on a `_get_minimum_size()` override.** `Button`/`BaseButton` computes its own minimum size from its native text/icon and does **not** factor in child nodes added by script — a `_get_minimum_size()` override was tried for `CoinChip` and `ProductCard` and was not reliably picked up by `HBoxContainer`/`GridContainer` layout passes, causing real overlap/collapse bugs (`CoinChip` overflowing into a sibling icon button; `ProductCard`s collapsing to near-zero width in a 2-column `GridContainer`). Setting `custom_minimum_size` directly — the same technique the pre-existing `shop_screen.gd::_make_card`/`_make_remove_ads_card` already used — is the reliable fix.

`CoinChip` replaces two previously hand-duplicated scene trees (gameplay HUD's `CoinChip`, Shop's old `BalanceChip`) that hardcoded `Color(...)` literals matching `Palette.COIN`/`COIN_CORE` without reading them (`.tscn` sub-resources can't read a runtime `static var`). Only Home (new) and Shop (migrated) use the shared component; gameplay HUD's `CoinChip` was deliberately left alone — that node path is load-bearing for the hint-economy tests.

**Forcing a screen's size in a headless test must use `set_deferred("size", ...)`, not a direct assignment.** A screen root with full-rect opposite anchors (`anchor_right`/`anchor_bottom = 1.0`) silently reverts a direct `.size =` write after the next layout pass (Godot logs a warning); `check_blocks_and_gate.gd::_test_faz9_tall_aspect()` and `check_hint_economy.gd::_test_hud_fits_tall_aspect()` both rely on this because `DisplayServer.window_set_size()` does not reliably resize the real window in this headless test environment.
...
`LevelLibrary.LEVEL_COUNT = 155` (files `res://levels/level_01.tres`..`level_155.tres`) — 150 campaign levels plus 5 bonus levels starting at `LevelWorlds.FIRST_BONUS_ID = 151`. Levels 1-50 are the hand-authored library with strict, enforced mechanical arcs; the verifier dispatches its check strategy by **level content**, not a hardcoded table:
...
- **41-50**: obstacles (`ObstacleData`) arrive and are combined with breakable blocks — every level in this band carries both. Introduces `METAL_RING` (41) then `BOMB` (42), then combines the two — `ROTATING_WHEEL`/`MOVING_BAR` are deliberately deferred to 51+ so one band doesn't stack every new mechanic at once. No dedicated verifier branch needed since obstacle motion is baked transparently into `LevelSolver`'s per-frame simulation.
- **51-100**: continues the obstacle arc — `ROTATING_WHEEL` gets its own first-sees level (51), `MOVING_BAR` gets its own (56), then 57-100 combine all four kinds (ring/bomb/wheel/bar) with obstacle count and speed/travel/rotation values scaling up toward level 100 (the band's boss). Breakable blocks appear on a curated subset of this band (`expected_block_levels` in the test), not on every level. `SPEED_BOOST` is deliberately excluded from this band because its real gameplay effect (shattering nearby breakable blocks, `gameplay.gd::_on_hazard_triggered`) is never simulated by `LevelSolver`, so a level relying on it to open a route could never be verified offline.
- **101-125**: the `ROTATING_WHEEL` band — every level contains at least one wheel, and obstacle kinds are restricted to ring/bomb/wheel. Only the levels listed in `wheel_block_levels` carry breakable blocks.
- **126-150**: the `PULSE_LASER` band — 126 is the readable introduction and a laser is present in every level after it; ring/bomb/wheel are reused alongside it. Blocks appear only on the levels listed in `laser_block_levels`.
- **151-155**: bonus levels, gated separately (`LevelWorlds.BONUS_GATE_RATIOS`) rather than by the campaign's sequential unlock.

`tools/check_blocks_and_gate.gd::_test_library_bounds()` is the authoritative programmatic check of this structure (only 1-25 are block-free; 26-50 all carry blocks and 30-40 include a durable one; 51-150 carry blocks only on the curated lists `expected_block_levels`/`wheel_block_levels`/`laser_block_levels`; 41+ has a non-empty `obstacles` array; 101-125 must each contain a wheel; 126-150 must each contain a laser; every band asserts `three_star_max_shots`/`two_star_max_shots` stay within `max_lives`) — update it if you change an arc's boundaries.
...
`ObstacleData.Kind`: `METAL_RING`, `BOMB`, `ROTATING_WHEEL`, `MOVING_BAR`, `SPEED_BOOST`, `PULSE_LASER`.

**`PULSE_LASER`** (126+) is the only *time-varying hazard*. It is a beam between two emitters that cycles on/off: `motion_period` is the full cycle (reusing the moving-bar field — same concept, no new field), `pulse_on_ratio` is the fraction spent lit, `phase_degrees` offsets it so two lasers in one level alternate instead of reading as one thick beam. `pulse_on_ratio` is capped at 0.85 because a permanently-lit beam is a wall, not a timing puzzle — that job belongs to `MOVING_BAR`.

Adding it forced one architectural change: **`ObstacleGeometry.hazard_shapes()` now takes `seconds`**, and `LevelSolver` recomputes hazards per frame (`_hazard_shapes_for_frame`) instead of caching them once at `bind_space`. Before this, a hazard could not change over time. The frame→seconds formula is deliberately *identical* to `_dynamic_shapes_for_frame`'s, so a beam and a rotating wheel never drift relative to each other.

The beam stays faintly visible while dark and brightens just before igniting (`LevelObstacle._apply_laser_state`). This telegraph is required, not decorative: a beam that ignites with no warning makes the level a coin flip rather than a timing puzzle. Rings are real static physics bodies: a 24-segment trapezoidal annulus with **4 segments skipped at the top**, i.e. a horseshoe. 20 of 24 segments are solid, so the centre is reachable *only through the ~60° opening at the top* — a ball arriving from below hits the metal body and bounces. (Do not describe this as "centre passable": the hollow centre is real geometry but unreachable from the launcher's direction, and the intro card previously taught the wrong rule because of that phrasing.) Bombs are `Area2D` hazards (`hazard_triggered` signal, failure reason `"bomb"`). Wheels/moving bars are `AnimatableBody2D` (`sync_to_physics = true`) driven by `ObstacleField.start_motion()`/`reset_motion()`, restarting phase at 0 every shot to match the solver's assumption. `LevelData.obstacles: Array[ObstacleData]` is additive and empty by default, so pre-obstacle levels are unaffected.
