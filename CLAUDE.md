...
- `check_obstacles.gd` — ring/bomb/wheel/moving-bar geometry, hazard detection, `.tres` round-trip, AI-obstacle mapping, and that levels 41-50 collectively exercise all four obstacle kinds.
...
Levels 1-50 (`LevelLibrary.LEVEL_COUNT = 50`, files `res://levels/level_01.tres`..`level_50.tres`) fall into distinct mechanical arcs, and the verifier dispatches its check strategy by **level content**, not a hardcoded table:
...
- **41-50**: obstacles (`ObstacleData` — rings, bombs, rotating wheels, moving bars), zero breakable blocks; no dedicated verifier branch needed since obstacle motion is baked transparently into `LevelSolver`'s per-frame simulation.

`tools/check_blocks_and_gate.gd::_test_library_bounds()` is the authoritative programmatic check of this structure (1-25 & 41-50 have no blocks; 26-40 do; 30-40 have a durable block; 41+ has a non-empty `obstacles` array) — update it if you change an arc's boundaries.
